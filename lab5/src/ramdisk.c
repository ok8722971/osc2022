...
void transfer_prog(char *from_addr, char *to_addr, unsigned long long fs) {
	unsigned char *new_bl = (unsigned char *)to_addr;
	unsigned char *bl = (unsigned char *)from_addr;
	while(fs--) {
		*new_bl++ = *bl++;
	}
}
