...
void sys_fork(struct trapframe *trapframe) {
    ...
    char *child_ustack = child_task->ustack_alloc + PAGE_SIZE - 16;
    char *parent_ustack = parent_task->ustack_alloc + PAGE_SIZE - 16;

    uint64_t kstack_offset = parent_kstack - (char*)trapframe;
	uint64_t ustack_offset = parent_ustack - (char*)trapframe->sp_el0;
	//uart_printf_sync("kfp:%D, ufp:%D\n", parent_kstack, parent_ustack);
	
	//uart_printf_sync("ksp:%D, usp:%D\n", (char*)trapframe, (char*)trapframe->sp_el0);
	//uart_printf_sync("k off:%D, u off:%D\n", kstack_offset, ustack_offset);
